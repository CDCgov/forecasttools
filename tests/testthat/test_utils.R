...
test_that("sym_limits and sym_limits_log agree with manual expectation", {
    ...
    expect_equal(sym_limits(c(-5, .52, 8)),
                 c(-8, 8))
    expect_equal(sym_limits(c(7), center = 2),
                 c(-3, 7))
    expect_equal(sym_limits(c(-5, .52, 8), center = 5),
                 c(-5, 15))
    expect_equal(sym_limits_log(c(100, 2, 0.00002)),
                 c(0.00002, 1 / 0.00002))
    expect_equal(sym_limits_log(c(100, 2, 0.00002), center = 2),
                 c(0.00002, 2 * 2 / 0.00002))
    expect_equal(sym_limits_log(c(1 / 5)),
                 c(1 / 5, 5))

    expect_equal(sym_limits_log(c(0, 2, 0.00002), center = 2),
                 c(0, Inf))
    expect_equal(sym_limits_log(c(0, 2, 0.00002)),
                 c(0, Inf))
...
test_that("sym_limits functions argument checks work", {
    ...
    expect_error(sym_limits(c(1.3, "a")),
                 "character")
    expect_error(sym_limits(c()),
                 "NULL")
    expect_error(sym_limits_log(c(1.5, "a")),
                 "character")
    expect_error(sym_limits_log(c(-5, .52, 8)),
                 ">= 0")
})
