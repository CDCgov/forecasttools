test_that(
    ...
    summary_test_case <- function(to_score,
                                  baseline,
                                  compare,
                                  metric_to_compare,
                                  by,
        ...
        scaled_rel_skill_col <- glue::glue("{metric_to_compare}",
        ...
        expect_equal(auto_baseline$mean_scores_ratio,
                     rep(1, nrow(auto_baseline)))
        expect_equal(auto_baseline[[scaled_rel_skill_col]],
                     rep(1, nrow(auto_baseline)))
        ...
        manual <- scores |>
            ...
            scoringutils:::as_scores(metrics = c(
                                        scoringutils::get_metrics(scores),
                                        "baseline_metric")) |>
            scoringutils::summarise_scores(by = c(compare, by)) |>
            dplyr::mutate(mean_scores_ratio =
                              .data[[metric_to_compare]] /
        ...
        expect_equal(auto |>
                     ...
                     dplyr::arrange(.data[[compare]],
                                    dplyr::across(tidyselect::any_of(by))) |>
                     dplyr::select(sort(tidyselect::peek_vars())),
                     dplyr::arrange(manual,
                                    .data[[compare]],
                                    dplyr::across(tidyselect::any_of(by))) |>
                     dplyr::select(sort(tidyselect::peek_vars())))
    }
