...
First, we create a table of forecast predictions formatted to work with `scoringutils` functions using `hub_to_scoreable_quantiles()`. Generally, we expect users to use `hub_to_scoreable_quantiles()` with a local path to the forecast repository which updates from GitHub by default. In this case, we download the hub first.
...
```{r}
forecast_and_target <- hub_to_scoreable_quantiles(hub_path)
```
