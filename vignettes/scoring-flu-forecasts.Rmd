...
```
`hub_to_scorable_quantiles()` performs a [left join](https://dplyr.tidyverse.org/reference/mutate-joins.html) of the forecast data to the target data, so the table includes forcasts for which there is no available evaluation data, with the `observed` column set to `NA`. `scoringutils` handles this.
